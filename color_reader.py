...
class ColorReader(Widget):
    ...
    spotButtonText = ReferenceListProperty(text1, text2, text3, text4,
    ...
    def addTextureAndResizeColorReader(self, tex):
        ...
        for inst in self.parent.canvas.children:
            if isinstance(inst, type(Rectangle())):
    ...
    def moveBox(self, *args):
        ...
        for x in self.currentSpot.instGrp.children:
            if isinstance(x, type(Rectangle())):
    ...
    def readSpot(self, image, spot):
        ...
        xRatio = image.size[0] / float(self.width)
        yRatio = image.size[1] / float(self.height)
        scaled_x = int((spotX - self.x) * xRatio)
        scaled_y = int((spotY - self.y) * yRatio)
        scaled_spotWidth = max(int(spotSize * xRatio), 1)
        scaled_spotHeight = max(int(spotSize * yRatio), 1)
        ...
        for i in [(5, -1, 0), (7, 0, 1), (9, 1, 0), (11, 0, -1)]:
            ...
            for j in range(0, scanRange, int(scanRange / 5)):
                ...
                croppedImage = image.crop((scaled_x, scaled_y,
                                           scaled_x + scaled_spotWidth,
                                           scaled_y + scaled_spotHeight))
                pixelData = (imageStat(croppedImage).mean[channelIndex],
                             spotX, spotY)
                colorValsList.append(pixelData)
